 ...
 * 
 * The double pointer causes the matrix to be called by reference, so nothing needs to be returned to shuffle the matrix.
 * 
 * ALSO
 * 
...
void shuffle1DMatrix(Square* matrix)
{
    ...
    for(passNumber; passNumber < 50000; passNumber++)
    {
        ...
        tmp = matrix[(randRow*ROWS) + randColumn];
        matrix[(randRow*ROWS) + randColumn] = matrix[(randRow2*ROWS) + randColumn2];
        matrix[(randRow2*ROWS) + randColumn2] = tmp;
    }
}